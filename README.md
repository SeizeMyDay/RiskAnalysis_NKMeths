...
이때, 위에서 볼 수 있다시피 이전 년도 동일 월의 압수현황 또한 표에 나타나 있다.  
여기서 문제가 생겼다. 이를테면 2001년 9월에 발간된 자료에서 나타난 메스암페타민 압수량은 57,457g이지만, 2002년 9월에 발간된 자료에서 나타난 2001년 9월의 메스암페타민 압수량은 54,085g이었다.  
두 데이터는 같아야만 하는데 달랐던 것이다. 이런 경우를 16개 확인하였고, 대검찰청에 다시 문의를 하게 되었다.  

그렇게 대검찰청 측의 확인을 받아 서로 달랐던 압수량 중에서 맞을 가능성이 더 높은 압수량을 선택해 독립변수의 원본 데이터로 활용하였다.  
...
### 종속변수: 홍콩 대한국 수출액 데이터
북한에서 남한으로 직접 마약을 전달하는 것은 극도로 어렵기 때문에, 북한발 마약을 남한에 유통하려면 경유지를 거치는 방법이 일반적으로 합리적인 선택지일 것이다.  
...
이 조건을 만족하는 경유지 후보는 상하이, 홍콩, 다롄, 칭다오, 톈진 등이 있었다.  
이 지역은 모두 한국과 북한에 동시에 인접한 중국의 항구도시라는 공통점이 있다.  

이 모든 지역의 대한국 수출량을 수집하여 대검찰청의 메스암페타민 압수 현황과 상관관계를 측정한 것을 위험도 지표로 삼아 서로 비교해 보려고 했었다.  
...
### 분석
분석 로직은 다음과 같다.  
1. 주어진 데이터의 기간에 맞게 Sliding Window를 구성한다. Window의 크기는 24개월로 하였다.
2. 하나의 Sliding Window에서 0개월부터 12개월까지 단방향 시차를 적용한 상관관계 검정을 시행한다. 이때 OLS의 추정량에 Newey-West Estimator를 적용한다.
3. 0개월부터 12개월까지 시차를 적용한 13개의 상관관계 분석결과를 비교하여 가장 적절한 시차의 모델을 선택한다.
4. 선택한 모델을 활용하여 해당 Window 중앙 시점의 메스암페타민 유통 위험도 지표를 산정한다. 위험도 지표는 모델의 beta값으로 하되, 다음의 필터를 두어 지표가 유의하지 않은 경우 beta값 대신 0을 위험도로 산정한다.
    1) 모델의 p-value가 0.05 이상일 때: 지표 자체가 통계적으로 유의하지 않아 신뢰할 수 없다.
    2) beta값이 음수일 때: 홍콩과 한국의 교역량 규모가 크다는 점을 고려할 때, 수출액과 밀수량 사이에 음의 상관관계가 존재하는 경우는 상식적이지 않다. 따라서 노이즈로 인한 모델의 오류일 가능성이 높다.
5. 2-4 과정을 1기씩 Window를 이동하며 반복한다. 각 Window 중앙 시점의 위험도 지표를 저장한다.
...
나는 이 지표가 정확하다고 생각하지 않는다.  
민간인으로 위장한 개인적 운반은 마약 밀반입의 주 경로 중 하나라고 알려져 있다.  
이 분석은 그런 방식의 운송량을 고려하지 못했다.  

이번에 나온 결과에서 홍콩-한국간 메스암페타민 관련 기사가 있는데도 위험도가 낮게 나온 경우는 찾아보기 어려웠다.  
따라서 이번 결과상으로는 안전한 지표라 할 수 있다.  
그러나 회귀분석, 그것도 moving window를 적용한 회귀분석은 그 특성상 데이터의 성질 변화나 분석 방법의 미세한 조절에 영향을 크게 받는다.  
그렇기에 이번 결과의 안전성이 앞으로도 강건할 것이라고 확신하기 어렵다.

또한 모델은 실제로 위험도가 낮게 도출되었을 경우 뿐 아니라 도출한 위험도가 유의하지 않을 경우에도 위험도 지표를 0으로 산정한다.  
따라서 모델이 도출한 결과값이 0이라면, 그 의미는 실제로 위험도가 낮다는 것이 아닌 위험도가 높은지 낮은지 알 수 없다는 의미로 받아들여야 한다.
이런 모델의 특성을 고려하면 그 강건성을 더욱 확신할 수 없다.  
모델이 도출한 위험도 지표는 측정 시기와 window 사이즈 변경, 회귀분석 모델 등의 요소에 따라 들쭉날쭉할 수 있다.  
